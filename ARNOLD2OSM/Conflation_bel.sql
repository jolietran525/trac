...
CREATE TABLE jolie_bel.osm_sw AS (
	SELECT *
...
ALTER TABLE jolie_bel.osm_sw RENAME COLUMN way TO geom;



-- points
CREATE table jolie_bel.osm_point AS (
	SELECT *
	FROM planet_osm_point
	WHERE
			way && st_setsrid( st_makebox2d( st_makepoint(-13603442,6043723), st_makepoint(-13602226,6044848)), 3857) ); -- 719

ALTER TABLE jolie_bel.osm_point RENAME COLUMN way TO geom;

-- highway = footway, footway = crossing
CREATE TABLE jolie_bel.osm_crossing AS (
	SELECT *
	FROM planet_osm_line
	WHERE   highway = 'footway' AND 
...
ALTER TABLE jolie_bel.osm_crossing RENAME COLUMN way TO geom;


-- highway = footway, footway IS NULL OR footway not sidewalk/crossing
CREATE TABLE jolie_bel.osm_footway_null AS (
	SELECT *
	FROM planet_osm_line
	WHERE 	highway = 'footway' AND 
...
ALTER TABLE jolie_bel.osm_footway_null RENAME COLUMN way TO geom;


--------- ARNOLD in bellevue ----------
CREATE TABLE jolie_bel.arnold_roads AS (
	SELECT *
...
-- This code creates a new table named segment_test by performing the following steps:
	-- Define intersection_points table which finds distinct intersection points between different geometries in the jolie_bel.arnold_roads table.
	...
	-- The resulting linestrings are grouped by object ID, route ID, and original geometry (a.objectid, a.routeid, a.geom) and inserted into the jolie_bel.arnold_segments_collection table.
CREATE TABLE jolie_bel.arnold_segments_collection AS
	WITH intersection_points AS (
	  ...
	  FROM jolie_bel.arnold_roads m1
	  JOIN jolie_bel.arnold_roads m2
...
CREATE TABLE jolie_bel.arnold_segments_line AS
	SELECT objectid, og_objectid, routeid, beginmeasure, endmeasure, (ST_Dump(st_split)).geom::geometry(LineString, 3857) AS geom
	FROM jolie_bel.arnold_segments_collection
	ORDER BY routeid, beginmeasure, endmeasure;

-- Create a geom index
CREATE INDEX segments_line_geom ON jolie_bel.arnold_segments_line USING GIST (geom);
CREATE INDEX sw_geom ON jolie_bel.osm_sw USING GIST (geom);
CREATE INDEX crossing_geom ON jolie_bel.osm_crossing USING GIST (geom);
CREATE INDEX point_geom ON jolie_bel.osm_point USING GIST (geom);
CREATE INDEX footway_null_geom ON jolie_bel.osm_footway_null USING GIST (geom);
...
SELECT *
FROM jolie_bel.osm_sw sidewalk; -- there ARE 255 sidewalks

-- check point 2.2: how many road segments are there
SELECT *
FROM jolie_bel.arnold_segments_line; -- there ARE 59 segments
...
-- new one
CREATE TABLE jolie_conflation_bel.big_sw AS
WITH ranked_roads AS (
	...
	FROM jolie_bel.osm_sw sidewalk
	JOIN jolie_bel.arnold_roads big_road ON ST_Intersects(ST_Buffer(sidewalk.geom, 10), ST_Buffer(big_road.geom, 30))
	WHERE 
...
CREATE INDEX big_sw_sidwalk_geom ON jolie_conflation_bel.big_sw USING GIST (osm_geom);
CREATE INDEX big_sw_arnold_geom ON jolie_conflation_bel.big_sw USING GIST (arnold_geom);
...
WHERE sidewalk.geom NOT IN (
	...
	FROM jolie_conflation_bel.big_sw big_sw); --71
	
---- STEP 3.1: Dealing with crossing
...
FROM jolie_bel.osm_crossing crossing -- 114

CREATE TABLE jolie_conflation_bel.crossing (osm_id, arnold_routeid, arnold_beginmeasure, arnold_endmeasure, osm_geom) AS
	...
	FROM jolie_bel.osm_crossing crossing
	LEFT JOIN jolie_bel.arnold_segments_line road
...
-- check point: segments (footway=sidewalk) intersects crossing
SELECT crossing.osm_id AS cross_id, sw.geom AS link_geom, crossing.osm_geom AS cross_geom, sw.osm_id AS link_id
FROM jolie_conflation_bel.crossing crossing
JOIN jolie_bel.osm_sw sw ON ST_Intersects(crossing.osm_geom, sw.geom)
WHERE ST_Length(sw.geom) < 10



CREATE TABLE jolie_conflation_bel.connlink (
	osm_id INT8,
...
INSERT INTO jolie_conflation_bel.connlink (osm_id, arnold_routeid1, arnold_beginmeasure1, arnold_endmeasure1, arnold_routeid2, arnold_beginmeasure2, arnold_endmeasure2, osm_geom)
WITH link_not_distinct AS (
    SELECT link_id, arnold_routeid, arnold_beginmeasure, arnold_endmeasure, cross_id, link_geom, cross_geom, ROW_NUMBER() OVER (PARTITION BY link_id ORDER BY arnold_routeid) AS rn
    FROM
        (
            SELECT
            ...
            FROM
                jolie_conflation_bel.crossing crossing
            JOIN
                jolie_bel.osm_sw sw ON ST_Intersects(crossing.osm_geom, st_startpoint(sw.geom)) OR ST_Intersects(crossing.osm_geom, st_endpoint(sw.geom))
...
FROM
    link_not_distinct
GROUP BY
    link_id, link_geom ; --41



-- check point: how highway=footway and footway IS NULL vs crossing are connected?
SELECT link.osm_id AS link_id, crossing.osm_id AS cross_id, crossing.arnold_routeid, crossing.arnold_beginmeasure, crossing.arnold_endmeasure, crossing.osm_geom AS cross_geom, link.geom AS link_geom
FROM jolie_bel.osm_footway_null link
JOIN jolie_conflation_bel.crossing crossing
...
INSERT INTO jolie_conflation_bel.connlink (osm_id, arnold_routeid1, arnold_beginmeasure1, arnold_endmeasure1, arnold_routeid2, arnold_beginmeasure2, arnold_endmeasure2, osm_geom)
	WITH link_not_distinct AS (
	SELECT link.osm_id AS link_id, crossing.osm_id AS cross_id, crossing.arnold_routeid, crossing.arnold_beginmeasure, crossing.arnold_endmeasure, crossing.osm_geom AS cross_geom, link.geom AS link_geom, ROW_NUMBER() OVER (PARTITION BY link.osm_id ORDER BY crossing.arnold_routeid) AS rn
		FROM jolie_bel.osm_footway_null link
		JOIN jolie_conflation_bel.crossing crossing
		ON ST_Intersects(link.geom, crossing.osm_geom)  
		WHERE ST_Length(link.geom) < 12
...
SELECT *
FROM jolie_conflation_bel.connlink 

-- remove osm_id in big_sw if it is identified as connlink
DELETE FROM jolie_conflation_bel.big_sw sw
WHERE sw.osm_id IN (
	SELECT bs.osm_id
	FROM jolie_conflation_bel.big_sw bs
	JOIN jolie_conflation_bel.connlink link
	ON link.osm_id = bs.osm_id 
)
...
CREATE TABLE jolie_conflation_bel.sw_edges (
	osm_id INT8,
	arnold_routeid1 VARCHAR(75),
	arnold_beginmeasure1 FLOAT8,
	arnold_endmeasure1 FLOAT8,
...
INSERT INTO jolie_conflation_bel.sw_edges (osm_id, arnold_routeid1, arnold_beginmeasure1, arnold_endmeasure1, arnold_routeid2, arnold_beginmeasure2, arnold_endmeasure2, osm_geom)
	...
	FROM jolie_bel.osm_sw edge
	JOIN jolie_conflation_bel.big_sw centerline1 ON st_intersects(st_startpoint(edge.geom), centerline1.osm_geom)
	JOIN jolie_conflation_bel.big_sw centerline2 ON st_intersects(st_endpoint(edge.geom), centerline2.osm_geom)
	WHERE   edge.geom NOT IN (
				SELECT sw.osm_geom
				FROM jolie_conflation_bel.big_sw sw)
			AND edge.geom NOT IN (
				SELECT link.osm_geom
				FROM jolie_conflation_bel.connlink link)
...
--CREATE TABLE jolie_conflation_bel.entrances AS
--	SELECT entrance.osm_id, sidewalk.osm_id AS sidewalk_id, entrance.geom AS osm_geom, point.tags, point.geom
--	FROM jolie_bel.osm_point point
--	JOIN (	SELECT *
--			FROM jolie_bel.osm_sw sidewalk
--			WHERE sidewalk.geom NOT IN (
--					SELECT osm_geom
--					FROM jolie_conflation_bel.big_sw ) AND 
--				  sidewalk.geom NOT IN (
--					SELECT osm_geom
--					FROM jolie_conflation_bel.sw_edges )  ) AS entrance
--	ON ST_intersects(entrance.geom, point.geom)
--	JOIN jolie_conflation_bel.big_sw sidewalk ON ST_Intersects(entrance.geom, sidewalk.osm_geom)
...
CREATE TABLE jolie_conflation_bel.entrances AS
	...
	JOIN (	SELECT *
			...
			WHERE sidewalk.geom NOT IN (
				  ...
				  sidewalk.geom NOT IN (
					SELECT osm_geom
					FROM jolie_conflation_bel.sw_edges )  AND 
				  sidewalk.geom NOT IN (
					SELECT osm_geom
					FROM jolie_conflation_bel.connlink )) AS entrance
	ON ST_intersects(entrance.geom, point.geom)
	JOIN jolie_conflation_bel.big_sw sidewalk ON ST_Intersects((entrance.geom), sidewalk.osm_geom)
...
WITH conf_table AS (
		SELECT CAST(big_sw.osm_id AS varchar(75)) AS id, big_sw.osm_geom, 'sidewalk' AS label FROM jolie_conflation_bel.big_sw big_sw
	    UNION ALL
	    SELECT CAST(link.osm_id AS varchar(75)) AS id, link.osm_geom, 'link' AS label FROM jolie_conflation_bel.connlink link 
	    UNION ALL
	    SELECT CAST(crossing.osm_id AS varchar(75)) AS id, crossing.osm_geom, 'crossing' AS label FROM jolie_conflation_bel.crossing crossing
	    UNION ALL
	    SELECT CAST(edge.osm_id AS varchar(75)) AS id, edge.osm_geom, 'edge' AS label FROM jolie_conflation_bel.sw_edges edge
	    UNION ALL
	    SELECT CAST(entrance.osm_id AS varchar(75)) AS id, entrance.osm_geom, 'entrance' AS label FROM jolie_conflation_bel.entrances entrance
	    )
SELECT CAST(sw.osm_id AS varchar(75)) AS id, sw.geom, 'not conflated' AS label
FROM jolie_bel.osm_sw sw
LEFT JOIN conf_table ON sw.geom = conf_table.osm_geom
WHERE conf_table.osm_geom IS NULL
UNION ALL
SELECT arnold.routeid AS id ,arnold.geom, 'road' AS label
FROM jolie_bel.arnold_roads arnold
...
CREATE TABLE jolie_bel.weird_case AS
	WITH conf_table AS (
			SELECT CAST(big_sw.osm_id AS varchar(75)) AS id, big_sw.osm_geom, 'sidewalk' AS label FROM jolie_conflation_bel.big_sw big_sw
		    UNION ALL
		    SELECT CAST(link.osm_id AS varchar(75)) AS id, link.osm_geom, 'link' AS label FROM jolie_conflation_bel.connlink link 
...
WITH ranked_roads AS (
	...
	FROM jolie_bel.weird_case_seg sidewalk
	JOIN jolie_bel.arnold_roads big_road ON ST_Intersects(ST_Buffer(sidewalk.geom, 10), ST_Buffer(big_road.geom, 20))
	WHERE 
	  (ABS(DEGREES(ST_Angle(ST_LineSubstring( big_road.geom, LEAST(ST_LineLocatePoint(big_road.geom, ST_ClosestPoint(st_startpoint(sidewalk.geom), big_road.geom)) , ST_LineLocatePoint(big_road.geom, ST_ClosestPoint(st_endpoint(sidewalk.geom), big_road.geom))), GREATEST(ST_LineLocatePoint(big_road.geom, ST_ClosestPoint(st_startpoint(sidewalk.geom), big_road.geom)) , ST_LineLocatePoint(big_road.geom, ST_ClosestPoint(st_endpoint(sidewalk.geom), big_road.geom))) ), sidewalk.geom))) BETWEEN 0 AND 10 -- 0 
		 OR ABS(DEGREES(ST_Angle(ST_LineSubstring( big_road.geom, LEAST(ST_LineLocatePoint(big_road.geom, ST_ClosestPoint(st_startpoint(sidewalk.geom), big_road.geom)) , ST_LineLocatePoint(big_road.geom, ST_ClosestPoint(st_endpoint(sidewalk.geom), big_road.geom))), GREATEST(ST_LineLocatePoint(big_road.geom, ST_ClosestPoint(st_startpoint(sidewalk.geom), big_road.geom)) , ST_LineLocatePoint(big_road.geom, ST_ClosestPoint(st_endpoint(sidewalk.geom), big_road.geom))) ), sidewalk.geom))) BETWEEN 170 AND 190 -- 180
		 OR ABS(DEGREES(ST_Angle(ST_LineSubstring( big_road.geom, LEAST(ST_LineLocatePoint(big_road.geom, ST_ClosestPoint(st_startpoint(sidewalk.geom), big_road.geom)) , ST_LineLocatePoint(big_road.geom, ST_ClosestPoint(st_endpoint(sidewalk.geom), big_road.geom))), GREATEST(ST_LineLocatePoint(big_road.geom, ST_ClosestPoint(st_startpoint(sidewalk.geom), big_road.geom)) , ST_LineLocatePoint(big_road.geom, ST_ClosestPoint(st_endpoint(sidewalk.geom), big_road.geom))) ), sidewalk.geom))) BETWEEN 350 AND 360 ) -- 360
...
INSERT INTO jolie_conflation_bel.big_sw(osm_id, arnold_routeid, arnold_beginmeasure, arnold_endmeasure, osm_geom, arnold_geom)
...
FROM jolie_bel.osm_sw
JOIN jolie_conflation_bel.big_sw
...
WHERE osm_sw.geom NOT IN (
		SELECT big_sw.osm_geom FROM jolie_conflation_bel.big_sw big_sw
	    UNION ALL
	    SELECT link.osm_geom FROM jolie_conflation_bel.connlink link 
	    UNION ALL
	    SELECT crossing.osm_geom FROM jolie_conflation_bel.crossing crossing
	    UNION ALL
	    SELECT edge.osm_geom FROM jolie_conflation_bel.sw_edges edge )
