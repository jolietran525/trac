...
CREATE TABLE jolie_sdot2osm_seattle_1.adjacent_lines AS
	...
	WHERE	public.f_within_degrees(ST_Angle(p1.geom, p2.geom), 15); --2200
...
CREATE TABLE jolie_sdot2osm_seattle_1.osm_sidewalk_preprocessed AS (
	...
	WHERE NOT ST_IsClosed(geom) ); -- 47380
...
CREATE TABLE jolie_sdot2osm_seattle_1.sdot2osm_sw_raw AS (
	WITH ranked_roads AS (
		...
		ON ST_Intersects(ST_Buffer(sdot.geom, sdot.sw_width * 5, 'endcap=flat join=round'), ST_Buffer(osm.geom, 4, 'endcap=flat join=round'))
		WHERE  CASE
				  	WHEN (sdot.length > osm.length)
				  		THEN 
					...
					THEN
					  	  ST_Length(
						  	ST_Intersection (
						  		...
						  		, ST_Buffer(r1.osm_seg, r1.sw_width*6, 'endcap=flat join=round')))
						  / LEAST(
				  	 ...
				  	 THEN 
				  	 	  ST_Length(
						  	ST_Intersection (
						  		...
						  		, ST_Buffer(r2.osm_seg, r2.sw_width*6, 'endcap=flat join=round')))
						  / LEAST(
					 ...
					 WHEN r1.sdot_seg IS NULL AND r2.sdot_seg IS NULL
				  	 THEN 
				  	 	  ST_Length(
				  	 	  	ST_Intersection(r1.osm_seg, ST_Buffer(r2.osm_seg, r2.sw_width*6, 'endcap=flat join=round') ) )
				  	 	  / LEAST(
				  	 	  	ST_Length(r1.osm_seg),
				  	 	  	ST_Length(r2.osm_seg))
				  	 	  > 0.4	 	
						...
						THEN 
							ST_Length(
						  		ST_Intersection(
						  			...
						  			, ST_Buffer(r1.sdot_seg, r1.sw_width*6, 'endcap=flat join=round')) )
						  	/LEAST(
						...
						THEN 
							ST_Length(
						  		ST_Intersection(
						  			...
						  			, ST_Buffer(r2.sdot_seg, r2.sw_width*6, 'endcap=flat join=round')) )
						  	/LEAST(
...
CREATE TABLE jolie_sdot2osm_seattle_1.sidewalk_json AS (
	...
	JOIN jolie_sdot2osm_seattle_1.osm_sidewalk osm
	...
	FROM jolie_sdot2osm_seattle_1.osm_sidewalk osm
	JOIN jolie_sdot2osm_seattle_1.osm_sidewalk_preprocessed pre_osm
