-- osm sidewalk
CREATE TABLE jolie_sdot2osm.osm_sidewalk_caphill AS
...
ALTER TABLE jolie_sdot2osm.osm_sidewalk_caphill RENAME COLUMN way TO geom;
CREATE INDEX sw_caphill_geom ON jolie_sdot2osm.osm_sidewalk_caphill USING GIST (geom);

-- osm crossing
CREATE TABLE jolie_sdot2osm.osm_crossing_caphill AS
	SELECT *, ST_Length(way) AS length
	FROM planet_osm_line pol 
...
ALTER TABLE jolie_sdot2osm.osm_crossing_caphill RENAME COLUMN way TO geom;
CREATE INDEX crossing_caphill_geom ON jolie_sdot2osm.osm_crossing_caphill USING GIST (geom);

-- osm connlink
CREATE TABLE jolie_sdot2osm.osm_connlink_caphill AS
	SELECT sw.*
	FROM jolie_sdot2osm.osm_crossing_caphill crossing
	JOIN jolie_sdot2osm.osm_sidewalk_caphill sw
	ON ST_Intersects(sw.geom, crossing.geom)
	WHERE sw.length < 12; -- 922
	
-- sdot sidewalk
CREATE TABLE sdot.sdot_sidewalk_caphill AS
...
CREATE INDEX osm_caphill_geom ON sdot.sdot_sidewalk_caphill USING GIST (geom);

-- conflation
CREATE TABLE jolie_sdot2osm.sdot2osm_caphill AS 
	WITH ranked_roads AS (
		...
		FROM jolie_sdot2osm.osm_sidewalk_caphill osm
		JOIN sdot.sdot_sidewalk_caphill sdot
		ON ST_Intersects(ST_Buffer(sdot.geom, sdot.sw_width/3.281, 'endcap=flat join=round'), osm.geom)
		WHERE  osm.osm_id NOT IN (SELECT osm_id FROM jolie_sdot2osm.osm_connlink_caphill) AND 
			   CASE
				  	WHEN (sdot.shape_length/3.281 > osm.length)
...
FROM jolie_sdot2osm.sdot2osm_caphill
...
SELECT *
FROM jolie_sdot2osm.sdot2osm_caphill s1
...
WHERE (geom) NOT IN 
	...
	--- full data
CREATE TABLE jolie_sdot2osm.osm_sidewalk AS
	SELECT *
	FROM planet_osm_line pol 
	WHERE   highway='footway' AND tags->'footway'='sidewalk' AND
			way && st_setsrid( st_makebox2d( st_makepoint(-13630776,6020942), st_makepoint(-13605961,6062738)), 3857)
			
ALTER TABLE jolie_sdot2osm.osm_sidewalk RENAME COLUMN way TO geom;
CREATE INDEX sw_geom ON jolie_sdot2osm.osm_sidewalk USING GIST (geom);
...
JOIN jolie_sdot2osm.osm_sidewalk osm
