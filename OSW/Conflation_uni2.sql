...
-- This code creates a new table named segment_test by performing the following steps:
	-- Define intersection_points table which finds distinct intersection points between different geometries in the jolie_uni2.arnold_roads table.
	...
	-- The resulting linestrings are grouped by object ID, route ID, and original geometry (a.objectid, a.routeid, a.geom) and inserted into the jolie_uni2.arnold_segments_collection table.
CREATE TABLE jolie_uni2.arnold_segments_collection AS
	WITH intersection_points AS (
	  ...
	  FROM jolie_uni2.arnold_roads m1
	  JOIN jolie_uni2.arnold_roads m2
...
CREATE TABLE jolie_uni2.arnold_segments_line AS
	SELECT objectid, og_objectid, routeid, beginmeasure, endmeasure, (ST_Dump(st_split)).geom::geometry(LineString, 3857) AS geom
	FROM jolie_uni2.arnold_segments_collection
	ORDER BY routeid, beginmeasure, endmeasure;

-- Create a geom index
CREATE INDEX segments_line_geom ON jolie_uni2.arnold_segments_line USING GIST (geom);
...
SELECT *
FROM jolie_uni2.osm_sw sidewalk; -- there ARE 184 sidewalks

-- check point 2.2: how many road segments are there
SELECT *
FROM jolie_uni2.arnold_segments_line; -- there ARE 59 segments
...
-- new one
CREATE TABLE jolie_conflation_uni2.big_sw AS
WITH ranked_roads AS (
	...
	JOIN jolie_uni2.arnold_roads big_road ON ST_Intersects(ST_Buffer(sidewalk.geom, 5), ST_Buffer(big_road.geom, 15))
	WHERE 
...
CREATE INDEX big_sw_sidwalk_geom ON jolie_conflation_uni2.big_sw USING GIST (osm_geom);
CREATE INDEX big_sw_arnold_geom ON jolie_conflation_uni2.big_sw USING GIST (arnold_geom);
...
WHERE sidewalk.geom NOT IN (
	...
	FROM jolie_conflation_uni2.big_sw big_sw); --119
...
CREATE TABLE jolie_conflation_uni2.sw_edges (
	osm_id INT8,
...
INSERT INTO jolie_conflation_uni2.sw_edges (osm_id, arnold_routeid1, arnold_beginmeasure1, arnold_endmeasure1, arnold_routeid2, arnold_beginmeasure2, arnold_endmeasure2, osm_geom)
	...
	JOIN jolie_conflation_uni2.big_sw centerline1 ON st_intersects(st_startpoint(edge.geom), centerline1.osm_geom)
	JOIN jolie_conflation_uni2.big_sw centerline2 ON st_intersects(st_endpoint(edge.geom), centerline2.osm_geom)
	WHERE   edge.geom NOT IN (
				SELECT sw.osm_geom
				FROM jolie_conflation_uni2.big_sw sw)
...
--CREATE TABLE jolie_conflation_uni2.entrances AS
...
--			WHERE sidewalk.geom NOT IN (
--					SELECT osm_geom
--					FROM jolie_conflation_uni2.big_sw ) AND 
--				  sidewalk.geom NOT IN (
--					SELECT osm_geom
--					FROM jolie_conflation_uni2.sw_edges )  ) AS entrance
--	ON ST_intersects(entrance.geom, point.geom)
--	JOIN jolie_conflation_uni2.big_sw sidewalk ON ST_Intersects(entrance.geom, sidewalk.osm_geom)
--	WHERE point.tags -> 'kerb' IS NULL
--			AND entrance.geom NOT IN (
--				SELECT osm_geom
--				FROM jolie_conflation_uni2.connlink
--			)
...
FROM jolie_uni2.osm_crossing crossing -- 60

CREATE TABLE jolie_conflation_uni2.crossing (osm_id, arnold_routeid, arnold_beginmeasure, arnold_endmeasure, osm_geom) AS
	...
	JOIN jolie_uni2.arnold_segments_line road ON ST_Intersects(crossing.geom, road.geom)
...
FROM jolie_conflation_uni2.crossing crossing
JOIN jolie_uni2.osm_sw sw ON ST_Intersects(crossing.osm_geom, sw.geom)
WHERE sw.geom NOT IN (
		SELECT osm_geom
		FROM jolie_conflation_uni2.big_sw ) AND
	  sw.geom NOT IN (
		SELECT osm_geom
		FROM jolie_conflation_uni2.sw_edges ) AND
	  ST_Length(sw.geom) < 10 -- leaving the two special cases OUT: osm_id = 490774566, 475987407
ORDER BY link_id



CREATE TABLE jolie_conflation_uni2.connlink (
	osm_id INT8,
	arnold_routeid1 VARCHAR(75), 
	arnold_beginmeasure1 FLOAT8,
	arnold_endmeasure1 FLOAT8,
...
INSERT INTO jolie_conflation_uni2.connlink (osm_id, arnold_routeid1, arnold_beginmeasure1, arnold_endmeasure1, arnold_routeid2, arnold_beginmeasure2, arnold_endmeasure2, osm_geom)
WITH link_not_distinct AS (
    ...
    FROM
        (
            ...
            FROM
                jolie_conflation_uni2.crossing crossing
            JOIN
                jolie_uni2.osm_sw sw ON ST_Intersects(crossing.osm_geom, sw.geom)
            WHERE
                sw.geom NOT IN (
                    SELECT osm_geom
                    FROM jolie_conflation_uni2.big_sw )
                AND sw.geom NOT IN (
					SELECT osm_geom
					FROM jolie_conflation_uni2.sw_edges )
...
JOIN (  SELECT link.osm_id AS link_id, crossing.osm_id AS cross_id, crossing.arnold_routeid, crossing.arnold_beginmeasure, crossing.arnold_endmeasure, crossing.osm_geom AS cross_geom, link.geom AS link_geom
		...
		JOIN jolie_conflation_uni2.crossing crossing
...
WITH conf_table AS (
		SELECT CAST(big_sw.osm_id AS varchar(75)) AS id, big_sw.osm_geom, 'sidewalk' AS label FROM jolie_conflation_uni2.big_sw big_sw
	    UNION ALL
	    SELECT CAST(link.osm_id AS varchar(75)) AS id, link.osm_geom, 'link' AS label FROM jolie_conflation_uni2.connlink link 
	    UNION ALL
	    SELECT CAST(crossing.osm_id AS varchar(75)) AS id, crossing.osm_geom, 'crossing' AS label FROM jolie_conflation_uni2.crossing crossing
	    UNION ALL
	    SELECT CAST(edge.osm_id AS varchar(75)) AS id, edge.osm_geom, 'edge' AS label FROM jolie_conflation_uni2.sw_edges edge
	    )
...
UNION ALL 
	SELECT routeid AS id, geom, 'road' AS label FROM jolie_uni2.arnold_roads;



SELECT count(*) FROM jolie_conflation_uni2.big_sw -- 65
SELECT count(*) FROM jolie_conflation_uni2.sw_edges -- 2
--SELECT count(*) FROM jolie_conflation_uni2.entrances --9
SELECT count(*) FROM jolie_conflation_uni2.connlink --68
...
SELECT  a.osm_id,
		(ST_Dump(ST_Split(a.geom, ST_Collect(b.geom)))).geom::geometry(LineString, 3857) AS segment,
		ROW_NUMBER() OVER (PARTITION BY a.osm_id ORDER BY (ST_Dump(ST_Split(a.geom, ST_Collect(b.geom)))).geom::geometry(LineString, 3857))
FROM jolie_uni2.osm_sw a
...
INSERT INTO jolie_conflation_uni2.big_sw(osm_id, arnold_routeid, arnold_beginmeasure, arnold_endmeasure, osm_geom, arnold_geom)
...
FROM jolie_uni2.osm_sw
JOIN jolie_conflation_uni2.big_sw
...
WHERE osm_sw.geom NOT IN (
		SELECT big_sw.osm_geom FROM jolie_conflation_uni2.big_sw big_sw
	    UNION ALL
	    SELECT link.osm_geom FROM jolie_conflation_uni2.connlink link 
	    UNION ALL
	    SELECT crossing.osm_geom FROM jolie_conflation_uni2.crossing crossing
	    UNION ALL
	    SELECT edge.osm_geom FROM jolie_conflation_uni2.sw_edges edge )
