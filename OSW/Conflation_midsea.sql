...
CREATE table jolie_midsea.osm_sw AS (
	SELECT *
	FROM planet_osm_line
	WHERE	highway = 'footway' AND
			tags -> 'footway' = 'sidewalk' AND 
			way && st_setsrid( st_makebox2d( st_makepoint(-13628406,6031754), st_makepoint(-13609490,6048075)), 3857) ); -- 20867

ALTER TABLE jolie_midsea.osm_sw RENAME COLUMN way TO geom;


-- points
CREATE table jolie_midsea.osm_point AS (
	SELECT *
	FROM planet_osm_point
	WHERE
			way && st_setsrid( st_makebox2d( st_makepoint(-13628406,6031754), st_makepoint(-13609490,6048075)), 3857) ); -- 78605

ALTER TABLE jolie_midsea.osm_point RENAME COLUMN way TO geom;

-- highway = footway, footway = crossing
CREATE TABLE jolie_midsea.osm_crossing AS (
	SELECT *
	FROM planet_osm_line
	WHERE   highway = 'footway' AND 
			tags -> 'footway' = 'crossing' AND 
			way && st_setsrid( st_makebox2d( st_makepoint(-13628406,6031754), st_makepoint(-13609490,6048075)), 3857) ); -- 9879

ALTER TABLE jolie_midsea.osm_crossing RENAME COLUMN way TO geom;


-- highway = footway, footway IS NULL OR footway not sidewalk/crossing
CREATE TABLE jolie_midsea.osm_footway_null AS (
	SELECT *
	FROM planet_osm_line
	WHERE 	highway = 'footway' AND 
			(tags -> 'footway' IS NULL OR 
			tags -> 'footway' NOT IN ('sidewalk', 'crossing'))  AND
			way && st_setsrid( st_makebox2d( st_makepoint(-13628406,6031754), st_makepoint(-13609490,6048075)), 3857)  ); -- 4421
		
ALTER TABLE jolie_midsea.osm_footway_null RENAME COLUMN way TO geom;


--------- ARNOLD in u-district ----------
CREATE TABLE jolie_midsea.arnold_roads AS (
	SELECT *
	FROM arnold.wapr_linestring 
	WHERE geom && st_setsrid( st_makebox2d( st_makepoint(-13628406,6031754), st_makepoint(-13609490,6048075)), 3857)
	ORDER BY routeid, beginmeasure, endmeasure); -- 2695


CREATE TABLE jolie_midsea.traffic_signals AS (
	SELECT *
	FROM sdot.traffic_signals
	WHERE wkb_geometry && st_setsrid( st_makebox2d( st_makepoint(-13628406,6031754), st_makepoint(-13609490,6048075)), 3857) ); -- 673

ALTER TABLE jolie_midsea.traffic_signals RENAME COLUMN wkb_geometry TO geom;
...
-- This code creates a new table named segment_test by performing the following steps:
	-- Define intersection_points table which finds distinct intersection points between different geometries in the jolie_midsea.arnold_roads table.
	...
	-- The resulting linestrings are grouped by object ID, route ID, and original geometry (a.objectid, a.routeid, a.geom) and inserted into the jolie_midsea.arnold_segments_collection table.
CREATE TABLE jolie_midsea.arnold_segments_collection AS
	WITH intersection_points AS (
	  ...
	  FROM jolie_midsea.arnold_roads m1
	  JOIN jolie_midsea.arnold_roads m2
	...
	FROM
	  jolie_midsea.arnold_roads AS a
	JOIN
...
CREATE TABLE jolie_midsea.arnold_segments_line AS
	SELECT objectid, og_objectid, routeid, beginmeasure, endmeasure, (ST_Dump(st_split)).geom::geometry(LineString, 3857) AS geom
	FROM jolie_midsea.arnold_segments_collection
	ORDER BY routeid, beginmeasure, endmeasure; --10563

-- Create a geom index
CREATE INDEX segments_line_geom ON jolie_midsea.arnold_segments_line USING GIST (geom);
CREATE INDEX sw_geom ON jolie_midsea.osm_sw USING GIST (geom);
CREATE INDEX crossing_geom ON jolie_midsea.osm_crossing USING GIST (geom);
CREATE INDEX point_geom ON jolie_midsea.osm_point USING GIST (geom);
CREATE INDEX footway_null_geom ON jolie_midsea.osm_footway_null USING GIST (geom);
...
WITH ranked_roads AS (
	...
	JOIN jolie_midsea.arnold_roads big_road ON ST_Intersects(ST_Buffer(sidewalk.geom, 5), ST_Buffer(big_road.geom, 15))
	WHERE 
...
CREATE INDEX big_sw_sidwalk_geom ON jolie_conflation_midsea.big_sw USING GIST (osm_geom);
CREATE INDEX big_sw_arnold_geom ON jolie_conflation_midsea.big_sw USING GIST (arnold_geom);

SELECT *
FROM jolie_conflation_midsea.big_sw
...
WHERE sidewalk.geom NOT IN (
	...
	FROM jolie_conflation_midsea.big_sw big_sw); --119
...
CREATE TABLE jolie_conflation_midsea.sw_edges (
	osm_id INT8,
...
INSERT INTO jolie_conflation_midsea.sw_edges (osm_id, arnold_routeid1, arnold_beginmeasure1, arnold_endmeasure1, arnold_routeid2, arnold_beginmeasure2, arnold_endmeasure2, osm_geom)
	...
	FROM jolie_midsea.osm_sw edge
	JOIN jolie_conflation_midsea.big_sw centerline1 ON st_intersects(st_startpoint(edge.geom), centerline1.osm_geom)
	JOIN jolie_conflation_midsea.big_sw centerline2 ON st_intersects(st_endpoint(edge.geom), centerline2.osm_geom)
	WHERE   edge.geom NOT IN (
				SELECT sw.osm_geom
				FROM jolie_conflation_midsea.big_sw sw)
...
--CREATE TABLE jolie_conflation_midsea.entrances AS
...
FROM jolie_midsea.osm_crossing crossing -- 60

CREATE TABLE jolie_conflation_midsea.crossing (osm_id, arnold_routeid, arnold_beginmeasure, arnold_endmeasure, osm_geom) AS
	...
	FROM jolie_midsea.osm_crossing crossing
	JOIN jolie_midsea.arnold_segments_line road ON ST_Intersects(crossing.geom, road.geom)
...
FROM jolie_conflation_midsea.crossing crossing
JOIN jolie_midsea.osm_sw sw ON ST_Intersects(crossing.osm_geom, sw.geom)
WHERE sw.geom NOT IN (
		SELECT osm_geom
		FROM jolie_conflation_midsea.big_sw ) AND
	  sw.geom NOT IN (
		SELECT osm_geom
		FROM jolie_conflation_midsea.sw_edges ) AND
	  ST_Length(sw.geom) < 10 -- leaving the two special cases OUT: osm_id = 490774566, 475987407
ORDER BY link_id



CREATE TABLE jolie_conflation_midsea.connlink (
	osm_id INT8,
	arnold_routeid1 VARCHAR(75), 
	arnold_beginmeasure1 FLOAT8,
	arnold_endmeasure1 FLOAT8,
...
INSERT INTO jolie_conflation_midsea.connlink (osm_id, arnold_routeid1, arnold_beginmeasure1, arnold_endmeasure1, arnold_routeid2, arnold_beginmeasure2, arnold_endmeasure2, osm_geom)
WITH link_not_distinct AS (
    ...
    FROM
        (
            ...
            FROM
                jolie_conflation_midsea.crossing crossing
            JOIN
                jolie_midsea.osm_sw sw ON ST_Intersects(crossing.osm_geom, sw.geom)
            WHERE
                sw.geom NOT IN (
                    SELECT osm_geom
                    FROM jolie_conflation_midsea.big_sw )
                AND sw.geom NOT IN (
					SELECT osm_geom
					FROM jolie_conflation_midsea.sw_edges )
...
JOIN (  SELECT link.osm_id AS link_id, crossing.osm_id AS cross_id, crossing.arnold_routeid, crossing.arnold_beginmeasure, crossing.arnold_endmeasure, crossing.osm_geom AS cross_geom, link.geom AS link_geom
		FROM jolie_midsea.osm_footway_null link
		JOIN jolie_conflation_midsea.crossing crossing
...
WITH conf_table AS (
		SELECT CAST(big_sw.osm_id AS varchar(75)) AS id, big_sw.osm_geom, 'sidewalk' AS label FROM jolie_conflation_midsea.big_sw big_sw
	    UNION ALL
	    SELECT CAST(link.osm_id AS varchar(75)) AS id, link.osm_geom, 'link' AS label FROM jolie_conflation_midsea.connlink link 
	    UNION ALL
	    SELECT CAST(crossing.osm_id AS varchar(75)) AS id, crossing.osm_geom, 'crossing' AS label FROM jolie_conflation_midsea.crossing crossing
	    UNION ALL
	    SELECT CAST(edge.osm_id AS varchar(75)) AS id, edge.osm_geom, 'edge' AS label FROM jolie_conflation_midsea.sw_edges edge
	    )
SELECT CAST(sw.osm_id AS varchar(75)) AS id, sw.geom, 'not conflated' AS label
FROM jolie_midsea.osm_sw sw
LEFT JOIN conf_table ON sw.geom = conf_table.osm_geom
WHERE conf_table.osm_geom IS NULL
UNION ALL 
	SELECT routeid AS id, geom, 'road' AS label FROM jolie_midsea.arnold_roads;



SELECT count(*) FROM jolie_conflation_midsea.big_sw -- 65
SELECT count(*) FROM jolie_conflation_midsea.sw_edges -- 2
--SELECT count(*) FROM jolie_conflation_midsea.entrances --9
SELECT count(*) FROM jolie_conflation_midsea.connlink --68
...
INSERT INTO jolie_conflation_midsea.big_sw(osm_id, arnold_routeid, arnold_beginmeasure, arnold_endmeasure, osm_geom, arnold_geom)
...
FROM jolie_midsea.osm_sw
JOIN jolie_conflation_midsea.big_sw
...
WHERE osm_sw.geom NOT IN (
		SELECT big_sw.osm_geom FROM jolie_conflation_midsea.big_sw big_sw
	    UNION ALL
	    SELECT link.osm_geom FROM jolie_conflation_midsea.connlink link 
	    UNION ALL
	    SELECT crossing.osm_geom FROM jolie_conflation_midsea.crossing crossing
	    UNION ALL
	    SELECT edge.osm_geom FROM jolie_conflation_midsea.sw_edges edge )
