...
-- osm sidewalk
CREATE TABLE jolie_sdot2osm_seattle_3.osm_sidewalk AS
	SELECT *, ST_Length(way) AS length
	FROM planet_osm_line pol
	WHERE   highway='footway' AND tags->'footway'='sidewalk' AND	
			way && st_setsrid( st_makebox2d( st_makepoint(-13620572,6036340), st_makepoint(-13611936,6043766)), 3857); --14860


ALTER TABLE jolie_sdot2osm_seattle_3.osm_sidewalk RENAME COLUMN way TO geom;
CREATE INDEX sw_geom ON jolie_sdot2osm_seattle_3.osm_sidewalk USING GIST (geom);




-- sdot sidewalk
DROP TABLE jolie_sdot2osm_seattle_3.sdot_sidewalk;
CREATE TABLE jolie_sdot2osm_seattle_3.sdot_sidewalk AS
	SELECT  ogc_fid,
			objectid,
			...
			ST_Length((ST_Dump(wkb_geometry)).geom) AS length -- CHANGE: Rerun the length
	FROM sdot.sidewalks
	WHERE   st_astext(wkb_geometry) != 'LINESTRING EMPTY'
			AND surftype != 'UIMPRV'
			AND sw_width != 0
			AND wkb_geometry && st_setsrid( st_makebox2d( st_makepoint(-13620572,6036340), st_makepoint(-13611936,6043766)), 3857); -- 9148

			
CREATE INDEX sdot_seattle_geom ON jolie_sdot2osm_seattle_3.sdot_sidewalk USING GIST (geom);
...
CREATE TABLE jolie_sdot2osm_seattle_3.polygon_osm_break AS
	WITH segments AS (
	    SELECT 
		        osm_id,
	    ...
	    FROM 	(
	        	SELECT 
	       		...
	       		FROM 
	            		jolie_sdot2osm_seattle_3.osm_sidewalk
...
CREATE TABLE jolie_sdot2osm_seattle_3.adjacent_lines AS
	SELECT  p1.osm_id AS osm_id
			,p1.segment_number AS seg_a
			,p1.geom AS geom_a
			,p2.segment_number AS seg_b
			,p2.geom AS geom_b
	FROM 	jolie_sdot2osm_seattle_3.polygon_osm_break p1
	JOIN 	jolie_sdot2osm_seattle_3.polygon_osm_break p2
...
CREATE TABLE jolie_sdot2osm_seattle_3.adjacent_linestrings (
...
CREATE OR REPLACE PROCEDURE jolie_sdot2osm_seattle_3.stitch_segments()
LANGUAGE plpgsql AS $$
DECLARE 
	_rec record;
	_first record;
...
BEGIN 
	FOR _rec IN
		SELECT  *
		  FROM  jolie_sdot2osm_seattle_3.adjacent_lines
	 ...
	 LOOP 
		 IF _first IS DISTINCT FROM NULL OR _prev IS DISTINCT FROM NULL THEN	-- _prev or _first record exists 
		 	...
		 	IF _rec.osm_id = _osm_id THEN	-- working on same osm_id
		 		...
		 		IF _prev IS DISTINCT FROM NULL THEN
		 			...
		 			ELSE
			 			...
			 			INSERT INTO jolie_sdot2osm_seattle_3.adjacent_linestrings VALUES (_prev.osm_id, _prev.seg_a, _prev.seg_b, ST_Linemerge(ST_Union(_prev.geom_a, _prev.geom_b)));
			 			-- set _prev record since 'new' line part
			 			_prev := _rec;
			 		END IF;
		 		ELSIF _first IS DISTINCT FROM NULL AND _rec.seg_a = _first.seg_a THEN
		    ...
		    ELSE	-- NEW osm_id
		    	...
		    	IF _first IS DISTINCT FROM NULL THEN
		    		...
		    		ELSE
			    		...
			    		INSERT INTO jolie_sdot2osm_seattle_3.adjacent_linestrings VALUES (_first.osm_id, _first.seg_b, _first.seg_a, ST_Linemerge(ST_Union(_first.geom_a, _first.geom_b)));
			    	END IF;
		    	END IF;
...
CALL jolie_sdot2osm_seattle_3.stitch_segments();


SELECT * FROM jolie_sdot2osm_seattle_3.adjacent_linestrings;--61

-- 3. Finally, for those that's not adjacent and parallel to others, we also want to insert them into the adjacent_parallel table

INSERT INTO jolie_sdot2osm_seattle_3.adjacent_linestrings
	SELECT osm_id, segment_number AS start_seg, segment_number AS end_seg, geom
	FROM jolie_sdot2osm_seattle_3.polygon_osm_break
...
-- we will have the columns:
		...
		-- geom
		-- length
CREATE TABLE jolie_sdot2osm_seattle_3.osm_sidewalk_preprocessed AS (
	SELECT osm_id, CONCAT(start_seg, '-', end_seg) AS start_end_seg, geom, ST_Length(geom) AS length
	FROM jolie_sdot2osm_seattle_3.adjacent_linestrings
	
	UNION ALL 
	
	SELECT osm_id, 'null' AS start_end_seg, geom, ST_Length(geom) AS length
	FROM jolie_sdot2osm_seattle_3.osm_sidewalk
	WHERE NOT ST_IsClosed(geom) ); -- 14973

CREATE INDEX osm_geom ON jolie_sdot2osm_seattle_3.osm_sidewalk_preprocessed USING GIST (geom);
...
-- conflation
CREATE TABLE jolie_sdot2osm_seattle_3.sdot2osm_sw_raw AS (
	WITH ranked_roads AS (
		  ...
		  ROW_NUMBER() OVER (
		  	...
		  	ORDER BY
		  		CASE
				  	WHEN (sdot.length > osm.length)
				  		THEN 
				  			-- CHANGE: base upon the similarity between the geometries instead of overlapping of area, the smaller the distance, the more similar --
				  			-- This is the distance between the (1) subsegmented sdot with (2) the original osm
				  			ST_HausdorffDistance(ST_LineSubstring( sdot.geom, LEAST(ST_LineLocatePoint(sdot.geom, ST_ClosestPoint(st_startpoint(osm.geom), sdot.geom)) , ST_LineLocatePoint(sdot.geom, ST_ClosestPoint(st_endpoint(osm.geom), sdot.geom))), GREATEST(ST_LineLocatePoint(sdot.geom, ST_ClosestPoint(st_startpoint(osm.geom), sdot.geom)) , ST_LineLocatePoint(sdot.geom, ST_ClosestPoint(st_endpoint(osm.geom), sdot.geom))) ),
				  				osm.geom,
				  				0.25) + 
				  			-- This is the distance between the (1) subsegmented sdot with (2) the intersection between the subsegmented sdot and the original osm (aka the part of osm original that overlap with the buffer of the subsegmented sdot)
				  			ST_HausdorffDistance(ST_LineSubstring( sdot.geom, LEAST(ST_LineLocatePoint(sdot.geom, ST_ClosestPoint(st_startpoint(osm.geom), sdot.geom)) , ST_LineLocatePoint(sdot.geom, ST_ClosestPoint(st_endpoint(osm.geom), sdot.geom))), GREATEST(ST_LineLocatePoint(sdot.geom, ST_ClosestPoint(st_startpoint(osm.geom), sdot.geom)) , ST_LineLocatePoint(sdot.geom, ST_ClosestPoint(st_endpoint(osm.geom), sdot.geom))) ),
				  				ST_Intersection(osm.geom,
				  					ST_Buffer(ST_LineSubstring( sdot.geom, LEAST(ST_LineLocatePoint(sdot.geom, ST_ClosestPoint(st_startpoint(osm.geom), sdot.geom)) , ST_LineLocatePoint(sdot.geom, ST_ClosestPoint(st_endpoint(osm.geom), sdot.geom))), GREATEST(ST_LineLocatePoint(sdot.geom, ST_ClosestPoint(st_startpoint(osm.geom), sdot.geom)) , ST_LineLocatePoint(sdot.geom, ST_ClosestPoint(st_endpoint(osm.geom), sdot.geom))) ),
				  						15, 'endcap=flat join=round')),
				  				0.25)
				  			-- CHANGE: end --
				  	ELSE
							-- CHANGE: base upon the similarity between the geometries instead of overlapping of area, the smaller the distance, the more similar --
				  			-- This is the distance between the subsegmented osm with the original sdot
				  			ST_HausdorffDistance(ST_LineSubstring( osm.geom, LEAST(ST_LineLocatePoint(osm.geom, ST_ClosestPoint(st_startpoint(sdot.geom), osm.geom)) , ST_LineLocatePoint(osm.geom, ST_ClosestPoint(st_endpoint(sdot.geom), osm.geom))), GREATEST(ST_LineLocatePoint(osm.geom, ST_ClosestPoint(st_startpoint(sdot.geom), osm.geom)) , ST_LineLocatePoint(osm.geom, ST_ClosestPoint(st_endpoint(sdot.geom), osm.geom))) ),
				  				sdot.geom,
				  				0.25) + 
				  			-- This is the distance between the (1) subsegmented osm with (2) the intersection between the subsegmented osm and the original sdot (aka the part of sdot original that overlap with the buffer of the subsegmented osm)
				  			ST_HausdorffDistance(ST_LineSubstring( osm.geom, LEAST(ST_LineLocatePoint(osm.geom, ST_ClosestPoint(st_startpoint(sdot.geom), osm.geom)) , ST_LineLocatePoint(osm.geom, ST_ClosestPoint(st_endpoint(sdot.geom), osm.geom))), GREATEST(ST_LineLocatePoint(osm.geom, ST_ClosestPoint(st_startpoint(sdot.geom), osm.geom)) , ST_LineLocatePoint(osm.geom, ST_ClosestPoint(st_endpoint(sdot.geom), osm.geom))) ),
				  				ST_Intersection(sdot.geom,
		...
		FROM jolie_sdot2osm_seattle_3.osm_sidewalk_preprocessed osm
		JOIN jolie_sdot2osm_seattle_3.sdot_sidewalk sdot
	...
	SELECT
		  ...
		  CASE
		  	WHEN osm_seg IS NULL
		  		THEN ST_Length(ST_Intersection(sdot_seg, ST_Buffer(osm_geom, 18,  'endcap=flat join=round')))/GREATEST(ST_Length(sdot_seg), ST_Length(osm_geom))
		  	ELSE 
		  		ST_Length(ST_Intersection(osm_seg, ST_Buffer(sdot_geom, 18,  'endcap=flat join=round')))/GREATEST(ST_Length(osm_seg), ST_Length(sdot_geom))
		  END AS conflated_score,		
		  -- CHANGE: Add hausdorff metrics for the filtering out in the next step --
		  CASE
		  	WHEN osm_seg IS NULL
		  		THEN ST_HausdorffDistance(sdot_seg, osm_geom, 0.25)
		  	ELSE 
		  		ST_HausdorffDistance(osm_seg, sdot_geom, 0.25)
		  END AS hausdorff_subseg_og,
		  CASE
		  	WHEN osm_seg IS NULL
		  		THEN ST_HausdorffDistance(sdot_seg, ST_Intersection(osm_geom, ST_Buffer(sdot_seg, 18, 'endcap=flat join=round')), 0.25)
		  	ELSE 
		  		ST_HausdorffDistance(osm_seg, ST_Intersection(sdot_geom, ST_Buffer(osm_seg, 18, 'endcap=flat join=round')), 0.25)
		  END AS hausdorff_subseg_intsct_og,
		  -- CHANGE: end --
		  osm_seg,
		  osm_geom,
		  sdot_seg,
...
SELECT * FROM jolie_sdot2osm_seattle_3.sdot2osm_sw_raw;
...
CREATE TABLE jolie_sdot2osm_seattle_3.sdot2osm_sw_prepocessed AS (
	SELECT  *
	FROM 
	    jolie_sdot2osm_seattle_3.sdot2osm_sw_raw
	WHERE 
	    (CONCAT(osm_id, start_end_seg), sdot_objectid) NOT IN ( 
	    	-- case when the whole osm already conflated to one sdot, but then its subseg (osm) also conflated to another sdot
	        ...
	        SELECT 
				    ...
				    -- CHANGE: use the hausdorff_subseg_intsct_og to determine to filter out which segment
				    CASE
				        WHEN r1.hausdorff_subseg_intsct_og > r2.hausdorff_subseg_intsct_og
				        	THEN r1.sdot_objectid
				        WHEN r1.hausdorff_subseg_intsct_og < r2.hausdorff_subseg_intsct_og
				        	THEN r2.sdot_objectid
				    END AS sdot_objectid
				    -- CHANGE: end --
			FROM jolie_sdot2osm_seattle_3.sdot2osm_sw_raw r1
			JOIN (
			    SELECT *
			    FROM jolie_sdot2osm_seattle_3.sdot2osm_sw_raw
					...
					THEN
					  	  ST_Length(
						  	ST_Intersection (
						  		...
						  		, ST_Buffer(r1.osm_seg, 18, 'endcap=flat join=round')))
						  / LEAST(
				  	 ...
				  	 THEN 
				  	 	  ST_Length(
						  	ST_Intersection (
						  		...
						  		, ST_Buffer(r2.osm_seg, 18, 'endcap=flat join=round')))
						  / LEAST(
					 ...
					 WHEN r1.sdot_seg IS NULL AND r2.sdot_seg IS NULL
				  	 THEN 
				  	 	  ST_Length(
				  	 	  	ST_Intersection(r1.osm_seg, ST_Buffer(r2.osm_seg, 18, 'endcap=flat join=round') ) )
				  	 	  / LEAST(
				  	 	  	ST_Length(r1.osm_seg),
				  	 	  	ST_Length(r2.osm_seg))
				  	 	  > 0.4	 	
			...
			-- case when the whole sdot already conflated to one osm, but then its subseg (sdot) also conflated to another osm
			-- if these 2 osm overlap at least 40% of the shorter one between 2 of them, then we filter out the one that's further away from our sdot
			SELECT 
				-- CHANGE: use the hausdorff_subseg_intsct_og TO FILTER OUT instead
				    CASE
				        WHEN r1.hausdorff_subseg_intsct_og > r2.hausdorff_subseg_intsct_og
				        	THEN CONCAT(r1.osm_id, r1.start_end_seg)
				        WHEN r1.hausdorff_subseg_intsct_og < r2.hausdorff_subseg_intsct_og
						...
						THEN 
							ST_Length(
						  		ST_Intersection(
						  			...
						  			, ST_Buffer(r1.sdot_seg, 18, 'endcap=flat join=round')) )
						  	/LEAST(
						...
						THEN 
							ST_Length(
						  		ST_Intersection(
						  			...
						  			, ST_Buffer(r2.sdot_seg, 18, 'endcap=flat join=round')) )
						  	/LEAST(
...
-- CHANGE: Have to UNION the sdot Geometry, because they could be multilinestring to begin with!!
CREATE TABLE jolie_sdot2osm_seattle_3.sdot2osm_metrics_sdot AS (
	SELECT  sdot.objectid,
			COALESCE(conflated_sdot_seg_length/ST_Length(sdot.geom), 0) AS percent_conflated,
			ST_Transform(sdot.geom, 4326) AS geom
			-- conflated_sdot_seg
	FROM (SELECT objectid, ST_UNION(geom) AS geom, SUM(length)
		  FROM jolie_sdot2osm_seattle_3.sdot_sidewalk
		  GROUP BY objectid) sdot -- CHANGE: need TO UNION back those that we dump earlier on
	LEFT JOIN (
			SELECT sdot_objectid,
				   ST_UNION(
						CASE 
				   ...
				   ST_Length(ST_UNION(
						CASE 
							WHEN osm_seg IS NOT NULL
								...
								ST_LineSubstring( sdot_geom, LEAST(ST_LineLocatePoint(sdot_geom, ST_ClosestPoint(st_startpoint(osm_seg), sdot_geom)) , ST_LineLocatePoint(sdot_geom, ST_ClosestPoint(st_endpoint(osm_seg), sdot_geom))), GREATEST(ST_LineLocatePoint(sdot_geom, ST_ClosestPoint(st_startpoint(osm_seg), sdot_geom)) , ST_LineLocatePoint(sdot_geom, ST_ClosestPoint(st_endpoint(osm_seg), sdot_geom))) )
							ELSE sdot_seg
						END)) conflated_sdot_seg_length -- CHANGE: INSTEAD, just UNION the conflated geometries here!
			FROM jolie_sdot2osm_seattle_3.sdot2osm_sw_prepocessed
			GROUP BY sdot_objectid ) sdot2osm
	ON sdot.objectid = sdot2osm.sdot_objectid
	GROUP BY sdot.objectid, sdot2osm.sdot_objectid, conflated_sdot_seg_length, sdot.geom ); -- CHANGE: Add conflated_sdot_seg_length TO trhe GROUP BY clause 
...
CREATE TABLE jolie_sdot2osm_seattle_3.sdot2osm_metrics_osm AS (
		SELECT  osm.osm_id,
				COALESCE(conflated_osm_seg_length/ST_Length(osm.geom), 0) AS percent_conflated,
				ST_Transform(osm.geom, 4326) AS geom
				--ST_UNION(sdot2osm.conflated_osm_seg) AS osm_conflated_subseg
		FROM jolie_sdot2osm_seattle_3.osm_sidewalk osm
		LEFT JOIN (
					SELECT  osm_id,
						    ST_UNION(
								CASE 
							...
							ST_Length(ST_UNION(
								CASE 
									WHEN sdot_seg IS NOT NULL
										...
										ST_LineSubstring( osm_geom, LEAST(ST_LineLocatePoint(osm_geom, ST_ClosestPoint(st_startpoint(sdot_seg), osm_geom)) , ST_LineLocatePoint(osm_geom, ST_ClosestPoint(st_endpoint(sdot_seg), osm_geom))), GREATEST(ST_LineLocatePoint(osm_geom, ST_ClosestPoint(st_startpoint(sdot_seg), osm_geom)) , ST_LineLocatePoint(osm_geom, ST_ClosestPoint(st_endpoint(sdot_seg), osm_geom))) )
									ELSE osm_seg
								END)) conflated_osm_seg_length
					FROM jolie_sdot2osm_seattle_3.sdot2osm_sw_prepocessed
					GROUP BY osm_id
				) sdot2osm -- CHANGE: INSTEAD, just UNION the conflated geometries here!
		ON osm.osm_id = sdot2osm.osm_id
		GROUP BY osm.osm_id, sdot2osm.osm_id, conflated_osm_seg_length, osm.geom ); -- CHAGE: ADD conflated_osm_seg_length TO the GROUP BY clause

SELECT * FROM jolie_sdot2osm_seattle_3.sdot2osm_metrics_osm;



CREATE TABLE jolie_sdot2osm_seattle_3.sidewalk AS
	SELECT  osm_id,
			start_end_seg,
		    sdot_objectid,
		    sw_width AS width,
		    ...
		    osm_geom,
		    sdot_geom
	FROM jolie_sdot2osm_seattle_3.sdot2osm_sw_prepocessed; -- 7625, 0.065 secs
...
CREATE TABLE jolie_sdot2osm_seattle_3.sidewalk_json AS (
	...
	FROM jolie_sdot2osm_seattle_3.sidewalk sdot2osm
	LEFT JOIN jolie_sdot2osm_seattle_3.osm_sidewalk osm
...
/*                 CROSSING DATA                  */
----------------------------------------------------
CREATE TABLE jolie_sdot2osm_seattle_3.osm_crossing AS
	SELECT *
	FROM planet_osm_line pol 
...
ALTER TABLE jolie_sdot2osm_seattle_3.osm_crossing RENAME COLUMN way TO geom;
CREATE INDEX crossing_geom ON jolie_sdot2osm_seattle_3.osm_crossing USING GIST (geom);
			
 
			
CREATE TABLE jolie_sdot2osm_seattle_3.sdot_accpedsig AS
	SELECT *
...
ALTER TABLE jolie_sdot2osm_seattle_3.sdot_accpedsig RENAME COLUMN wkb_geometry TO geom;
CREATE INDEX geom ON jolie_sdot2osm_seattle_3.sdot_accpedsig USING GIST (geom);
...
FROM jolie_sdot2osm_seattle_3.osm_crossing osm 
JOIN jolie_sdot2osm_seattle_3.sdot_accpedsig sdot
ON ST_Intersects(ST_Buffer(osm.geom, 10, 'endcap=flat join=round'), ST_Buffer(sdot.geom, 20))


-- FINAL TABLE
-- changes: transform, buffer size
CREATE TABLE jolie_sdot2osm_seattle_3.crossing_json AS (
	SELECT 
